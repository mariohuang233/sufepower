...
def test_export_is_deidentified_and_atomic(tmp_path):
    ...
    assert 'PRIVATE-1' not in (target/'v1/manifest.json').read_text(encoding='utf-8')
    rooms=json.loads((target/'v1/registry/rooms.json').read_text(encoding='utf-8'))
    assert rooms[0]['balance_unit'] == '元'
    manifest=json.loads((target/'v1/manifest.json').read_text(encoding='utf-8'))
    assert manifest['collection_schedule'] == 'daily_23_shanghai'
    assert manifest['consumption_method'] == 'adjacent_daily_balance_drop'
...
def test_export_rehydrates_previous_public_history_for_ephemeral_runner(tmp_path):
    ...
    campus_daily=json.loads((target/'v1/daily/campuses/campus-1.json').read_text(encoding='utf-8'))
    assert campus_daily == daily
